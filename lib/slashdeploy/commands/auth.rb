  ...
  module Commands
    ...
    class Auth
      ...
      def call(env)
        ...
        team_domain = nil
        user_name = nil
        if cmd.request.user_id
          user_id = cmd.request.user_id
          team_id = cmd.request.team_id
          team_domain = cmd.request.team_domain
          user_name = cmd.request.user_name
        else
          user_id = env['action'].request.user.id
          team_id = env['action'].request.team.id
          team_domain = env['action'].request.team.domain
          user_name = env['action'].request.user.name
        end
